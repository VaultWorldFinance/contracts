...
contract SmartChef is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 accVaultwPerShare; // Accumulated VAULTWs per share, times 1e16. See below.
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accVaultwPerShare = accVaultwPerShare.add(vaultwReward.mul(1e16).div(lpSupply));
        }
        return user.amount.mul(accVaultwPerShare).div(1e16).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 vaultwReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        pool.accVaultwPerShare = pool.accVaultwPerShare.add(vaultwReward.mul(1e16).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _amount) public {
        ...
        updatePool(0);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accVaultwPerShare).div(1e16).sub(user.rewardDebt);
            if(pending > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accVaultwPerShare).div(1e16);
